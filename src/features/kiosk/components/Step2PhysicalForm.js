...
export function Step2PhysicalForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex w-full flex-col gap-6 tablet:gap-8">
      ...
      <FormCard
        title={copy.step2GenderTitle}
        description={copy.step2GenderSubtitle}
        className="p-4 tablet:p-5 kiosk:p-6 [&>header]:mb-3 tablet:[&>header]:mb-4"
      >
        <div className="grid grid-cols-1 gap-2.5 tablet:grid-cols-2 tablet:gap-3 kiosk:grid-cols-3 kiosk:gap-3">
        ...
        {errors.genderCategory ? (
          <p className="mt-3 text-center text-sm text-error">{errors.genderCategory.message}</p>
        ) : null}
      </FormCard>
